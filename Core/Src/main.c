...
uint8_t i2c_rx[PACKET_SIZE];
uint8_t i2c_tx[PACKET_SIZE];
...
int main(void)
{
	...
	while (1)
	{
		...
		{ /* Wait for timer interrupt to happen */
			...
			EventHandler_Peripherals(peripheralEvent);


			if (HAL_I2C_GetState(&hi2c1) == HAL_I2C_STATE_READY)
			{
				HAL_I2C_Slave_Receive_DMA(&hi2c1, i2c_rx, PACKET_SIZE);
			}
...
/* USER CODE BEGIN 4 */
void EventHandler_Peripherals(PeripheralEvent event)
{
	switch (event)
	{
